...
- Add a `/campaigns/:campaignId/terminals` list page scoped to the campaign in `CurrentCampaignService`. PrimeNG `<p-table>` with columns: title, public flag, action buttons (open detail, export, delete). Show an empty-state placeholder when no campaign is selected.
