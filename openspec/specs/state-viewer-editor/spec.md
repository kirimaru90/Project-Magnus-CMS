...
The terminal detail page (`/terminals/:id`) SHALL render a local-state panel below the content editor. The panel SHALL load state via `GET /terminals/:id/state` and display a PrimeNG table with columns `key`, `type`, `default`, and `current`, plus per-row actions. When the terminal declares no local variables, the panel SHALL render an empty state ("Nessuna variabile locale dichiarata") instead of an empty table.

#### Scenario: Local state renders

- **WHEN** an admin opens `/terminals/:id` for a terminal that declares local variables
- **THEN** the page calls `GET /terminals/:id/state` and shows one table row per declared local variable with its key, type, default value, and current value
...
A `StateApiService` SHALL expose every state read, mutate, and reset call (`getTerminalState`, `getCampaignState`, `mutateTerminal`, `mutateCampaign`, `resetTerminalVar`, `resetCampaignVar`, `resetTerminalAll`, `resetCampaignAll`). Panel and reset components SHALL NOT call `HttpClient` directly for state operations.

#### Scenario: Reads go through the service

- **WHEN** a state panel loads its data
- **THEN** the request is issued by `StateApiService`, not by a direct `HttpClient` call in a component

#### Scenario: State read body is validated

- **WHEN** a `GET .../state` response is received
- **THEN** it is parsed against the `StateEntryDto` Zod schema so contract drift surfaces as a parse error rather than a silent mis-render
